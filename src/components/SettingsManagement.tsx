...
export const SettingsManagement = () => {
  ...
  const [newPromo, setNewPromo] = useState({ code: '', discountPercent: 0, isActive: true });
  ...
  const addPromo = async () => {
    try {
      ...
      setNewPromo({ code: '', discountPercent: 0, isActive: true });
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="space-y-2 mb-4">
          ...
          <input type="number" placeholder="Discount %" value={newPromo.discountPercent} onChange={e => setNewPromo({...newPromo, discountPercent: Number(e.target.value)})} className="w-full p-2 border rounded" />
        ...
        {promoCodes.map(p => (
          <div key={p.id} className="flex justify-between items-center p-2 border rounded mb-2">
            <span>{p.code} - {p.discountPercent}%</span>
